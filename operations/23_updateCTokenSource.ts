...
(async () => {
  ...
  const cTokenSources = jsonToMap(fs.readFileSync('./output/cTokenSources.json', 'utf8')) as Map<string, string>;
  ...
  for (let [baseId, quoteId, oracleName, sourceAddress] of newSources) {
    ...
    const oracle = await ethers.getContractAt('CTokenMultiOracle', protocol.get(oracleName) as string, ownerAcc) as unknown as CTokenMultiOracle

    proposal.push({
      target: oracle.address,
      data: oracle.interface.encodeFunctionData("setSource", [baseId, quoteId, sourceAddress])
    })
    console.log(`[spot: ${bytesToString(baseId)}/${bytesToString(quoteId)}: ${cTokenSources.get(pairId) || undefined} -> ${sourceAddress}],`)
  ...
  fs.writeFileSync('./output/cTokenSources.json', mapToJson(cTokenSources), 'utf8')
})()
