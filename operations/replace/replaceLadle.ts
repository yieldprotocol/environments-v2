...
(async () => {
  ...
  proposal.push({
    ...
    data: wand.interface.encodeFunctionData('point', [
        ethers.utils.formatBytes32String('ladle'),
        ladle.address
    ])
  })
  ...
  for (let seriesId of fyTokens.keys()) {
    const fyToken = await ethers.getContractAt('FYToken', fyTokens.get(seriesId) as string, ownerAcc) as unknown as FYToken
  ...
  for (let symbol of strategies.keys()) {
    const strategy = await ethers.getContractAt('Strategy', strategies.get(symbol) as string, ownerAcc) as unknown as Strategy
