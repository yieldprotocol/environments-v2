...
;(async () => {
  ...
  proposal = proposal.concat(await updateChiSourcesProposal(ownerAcc, compoundOracle, chiSources))
  proposal = proposal.concat(await updateRateSourcesProposal(ownerAcc, compoundOracle, rateSources))
  proposal = proposal.concat(await updateChainlinkSourcesProposal(ownerAcc, chainlinkOracle, chainlinkSources))
  proposal = proposal.concat(await updateUniswapSourcesProposal(ownerAcc, uniswapOracle, uniswapSources))
  proposal = proposal.concat(await updateLidoSourceProposal(ownerAcc, lidoOracle, lidoSource))
  proposal = proposal.concat(await updateCompositeSourcesProposal(ownerAcc, compositeOracle, compositeSources))
  proposal = proposal.concat(await updateCompositePathsProposal(ownerAcc, compositeOracle, compositePaths))
