...
import { developer, deployer, poolData, poolFees, timeStretch } from './newEnvironment.rinkeby.config'
...
;(async () => {
  ...
  if ((await timelock.proposals(hash)).state === 0) {
    ...
    writeAddressMap('pools.json', pools) // newPools.json is a temporary file
  }
})()
