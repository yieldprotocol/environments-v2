...
;(async () => {
  ...
  for (let [seriesId] of seriesIlks) {
    ...
    const posted = (await oracle.peek(bytesToBytes32(series.baseId), bytesToBytes32(assetId), borrowed))[0].mul(ratio).div(1000000).mul(101).div(100) // borrowed * spot * ratio * 1.01 (for margin)
