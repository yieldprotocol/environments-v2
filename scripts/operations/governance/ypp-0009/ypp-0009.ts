...
;(async () => {
  ...
  const governance = jsonToMap(fs.readFileSync('../../../../addresses/archive/mainnet/governance.json', 'utf8')) as Map<
    string,
    string
  >
  ...
  if ((await timelock.proposals(txHash)).state === 0) {
    ...
    // Execute
    if (on_fork) {
      // If running on a mainnet fork
      console.log(`Running on a fork, will execute`)
