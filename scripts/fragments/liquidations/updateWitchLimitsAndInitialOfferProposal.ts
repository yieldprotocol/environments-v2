...
export const updateWitchLimitsAndInitialOfferProposal = async (
  witch: Witch,
  ...
  for (let [ilkId, initialOffer, line, dust, dec] of newLimits) {
    ...
    console.log(`${bytesToString(ilkId)}: ${ilk.initialOffer}/${limits.line}/${limits.dust}/${limits.dec} -> ${initialOffer}/${line}/${dust}/${dec}`)
  }
  return proposal
}
