...
export const deployFYTokens = async (
  ...
  for (let [seriesId, underlyingId, chiOracleAddress, joinAddress, maturity, name, symbol] of fyTokenData) {
    ...
    if ((await ethers.provider.getCode(chiOracleAddress)) === '0x') throw `Oracle at ${chiOracleAddress} contains no code`
    ...
    const fyToken = (await fyTokenFactory.deploy(underlyingId, chiOracleAddress, joinAddress, maturity, name, symbol)) as unknown as FYToken
