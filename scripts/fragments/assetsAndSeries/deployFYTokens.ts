...
const { newJoins } = require(process.env.CONF as string)
...
export const deployFYTokens = async (
  ...
  for (let [seriesId, underlyingId, chiOracleAddress, joinAddress, maturity, name, symbol] of fyTokenData) {
    if (joinAddress === undefined) {
      console.log('underlyingId', underlyingId)
      console.log(newJoins)
      joinAddress = newJoins.get(underlyingId)
      if (joinAddress === undefined) {
        throw `Join for ${seriesId} not found`
      }
    }
