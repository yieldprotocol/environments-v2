...
export const initPoolsForRollProposal = async (
  ownerAcc: any,
  timelock: Timelock,
  newPools: Map<string, string>,
  poolsInitForRoll: Array<[string, string, BigNumber, BigNumber, string, BigNumber]>
): Promise<Array<{ target: string; data: string }>> => {
  // Build the proposal
  const proposal: Array<{ target: string; data: string }> = []

  for (let [seriesId, baseId, baseAmount, fyTokenAmount, to, min] of poolsInitForRoll) {
    ...
    proposal.push({
      target: pool.address,
      data: pool.interface.encodeFunctionData('sellFYToken', [to, min]),
    })
    console.log(`Sold FYToken ${await pool.symbol()} with ${baseAmount} base and ${fyTokenAmount} fyToken`)
  }
