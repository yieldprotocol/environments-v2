...
export class StrategyData {
  constructor(readonly name: string, readonly symbol: string, readonly baseId: string) {}
}

export class StrategyInitData {
  constructor(readonly symbol: string, readonly startPoolId: string, readonly startSeriesId: string) {}
}

export async function deployStrategy(ctx: DeployedContext, strategyData: StrategyData): Promise<Strategy> {
  const base = await getContract<ERC20Mock>(
    ctx.owner,
    'ERC20Mock',
    await ctx.getRegisteredAssetAddress(strategyData.baseId)!
  )
  ...
  if (strategies.has(strategyData.symbol)) {
    ...
    verify(
      strategy.address,
      [strategyData.name, strategyData.symbol, ctx.ladle.address, base.address, strategyData.baseId],
      'safeERC20Namer.js'
    )
