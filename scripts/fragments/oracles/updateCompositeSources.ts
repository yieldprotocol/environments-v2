  ...
  for (let source of compositeSources) {
    ...
    if (existent === ZERO_ADDRESS || overrideExistent) {
      proposal.push({
        ...
        data: compositeOracle.interface.encodeFunctionData('setSource', [source.baseId, source.quoteId, source.source]),
      })
      console.log(`CompositeMultiOracle: pair: ${pair} set to ${source.source}`)
    } else {
