...
describe("Ladle - serve and repay", function () {
  ...
  it("test all", async () => {
    ...
    const pools = jsonToMap(
      fs.readFileSync("./output/pools.json", "utf8")
    ) as Map<string, string>;
    ...
    expect(await base.balanceOf(other)).to.equal(
      baseBalanceBefore.add(baseBorrowed)
    );
    expect(await ilk.balanceOf(owner)).to.equal(
      ilkBalanceBefore.sub(inkPosted)
    );
    ...
    expect(await base.balanceOf(owner)).to.equal(
      baseBalanceBefore.sub(debtRepaidInBase)
    );
