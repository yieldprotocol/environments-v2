...
describe("FYToken", function () {
  ...
  it("test all", async () => {
    ...
    const assets = jsonToMap(
      fs.readFileSync("./output/assets.json", "utf8")
    ) as Map<string, string>;
    const chiSources = jsonToMap(
      fs.readFileSync("./output/chiSources.json", "utf8")
    ) as Map<string, string>;
    const protocol = jsonToMap(
      fs.readFileSync("./output/protocol.json", "utf8")
    ) as Map<string, string>;
    const joins = jsonToMap(
      fs.readFileSync("./output/joins.json", "utf8")
    ) as Map<string, string>;
    const fyTokens = jsonToMap(
      fs.readFileSync("./output/fyTokens.json", "utf8")
    ) as Map<string, string>;

    innerLadle = (await ethers.getContractAt(
      "Ladle",
    ...
    baseJoin = (await ethers.getContractAt(
      "Join",
      joins.get(baseId) as string,
      ownerAcc
    )) as Join;
    ilk = (await ethers.getContractAt(
      "ERC20Mock",
      assets.get(ilkId) as string,
      ownerAcc
    )) as ERC20Mock;
    ilkJoin = (await ethers.getContractAt(
      "Join",
      joins.get(ilkId) as string,
      ownerAcc
    )) as Join;
    ...
    chiSource = (await ethers.getContractAt(
      "ISourceMock",
      chiSources.get(baseId) as string,
      ownerAcc
    )) as ISourceMock;
    ...
    await expect(fyToken.redeem(owner, WAD)).to.be.revertedWith(
      "Only after maturity"
    );

    console.log("after maturity");
    await ethers.provider.send("evm_mine", [
      (await fyToken.maturity()).toNumber(),
    ]);
    ...
    expect(await base.balanceOf(baseJoin.address)).to.equal(
      baseJoinBefore.sub(WAD.mul(accrual).div(WAD))
    );
    expect(await base.balanceOf(owner)).to.equal(
      baseOwnerBefore.add(WAD.mul(accrual).div(WAD))
    );
