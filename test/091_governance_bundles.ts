...
describe("Governance", function () {
  ...
  it("governance", async () => {
    ...
    base = (await deployContract(owner, ERC20MockArtifact, [
      baseId,
      baseId,
    ])) as ERC20Mock;
    ilk = (await deployContract(owner, ERC20MockArtifact, [
      ilkId,
      ilkId,
    ])) as ERC20Mock;
    spotSource = (await deployContract(
      owner,
      ChainlinkAggregatorV3MockArtifact,
      []
    )) as ISourceMock;
    rateSource = (await deployContract(
      owner,
      CTokenRateMockArtifact,
      []
    )) as ISourceMock;
    await rateSource.set(WAD.mul(2));
    chiSource = (await deployContract(
      owner,
      CTokenChiMockArtifact,
      []
    )) as ISourceMock;
    ...
    baseJoin = (await deployContract(owner, JoinArtifact, [
      base.address,
    ])) as Join;
    await ladle.addJoin(baseId, baseJoin.address);
    await baseJoin.grantRoles(
      [id("join(address,uint128)"), id("exit(address,uint128)")],
      ladle.address
    );
    // })
    ...
    // Deploy Join and integrate it with Ladle
    ilkJoin = (await deployContract(owner, JoinArtifact, [
      ilk.address,
    ])) as Join;
    ...
    await fyToken.grantRoles(
      [id("mint(address,uint256)"), id("burn(address,uint256)")],
      ladle.address
    );
    ...
    // 5a - Deploy Pool
    const calculatedAddress = await factory.calculatePoolAddress(
      base.address,
      fyToken.address
    );
