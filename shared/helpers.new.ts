...
export const proposeApproveExecute = async (
  ...
  if ((await timelock.proposals(txHash)).state === ProposalState.Unknown) {
    ...
    let signerAcc = await getOwnerOrImpersonate(developer as string, BigNumber.from('1000000000000000000'))
  ...
  } else if ((await timelock.proposals(txHash)).state === ProposalState.Proposed) {
    console.log('Approving')
    let signerAcc = await getOwnerOrImpersonate(multisig as string, BigNumber.from('1000000000000000000'))

    const tx = await timelock.connect(signerAcc).approve(txHash)
    await requireProposalState(tx, ProposalState.Approved)
    console.log(`Approved ${txHash}`)

    const eta = (await timelock.proposals(txHash)).eta
    console.log(`Advancing time to ${eta}`)
    advanceTimeTo(eta)
  } else if ((await timelock.proposals(txHash)).state === ProposalState.Approved) {
    console.log('Executing')
    let signerAcc = await getOwnerOrImpersonate(developer as string, BigNumber.from('1000000000000000000'))

    const gasEstimate = await timelock.connect(signerAcc).estimateGas.execute(proposal)
