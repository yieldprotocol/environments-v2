...
const { strategies, pools } = require(process.env.CONF as string)
...
;(async () => {
  ...
  const strategyAddresses = strategies.values() // This is the list of addresses that will be iterated through
  const poolAddresses = [...pools.values()];

  const factory = ERC20__factory
  for (let addr of strategyAddresses) {
    ...
    if (events.length) {
      ...
      for (let event of events) {
        if (poolAddresses.includes(event.args[1])) {
          const txHash = event.transactionHash
          const tx = await ethers.provider.getTransaction(txHash)
          console.log(tx.blockNumber, tx.from, event.transactionHash, /*event.args[0], event.args[1],*/ event.args[2].toString())
        }
      }
    }
  }
