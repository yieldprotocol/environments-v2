...
import { Witch__factory } from '../../typechain'
import { WITCH } from '../../shared/constants'
...
;(async () => {
  ...
  const filter = witch.filters.Auctioned(null, null, null, null)
  ...
  if (events.length) {
    for (let event of events) {
      ...
      console.log(tx.blockNumber, tx.from, event.transactionHash, event.args[0], event.args[1], event.args[2], event.args[3])
    }
  }
})()
